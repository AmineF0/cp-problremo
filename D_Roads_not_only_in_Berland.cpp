...
void solve(){
    ...
    vector<pair<int, int>> to_rep;

    for(int i=0; i<n-1; i++){
      int a, b; cin >> a >> b; a--,b--;
      if(dsu.connected(a, b)){
        to_rep.push_back({a,b});
      } else {
        dsu.merge(a, b);
      }
    }

    cout << to_rep.size() << endl;
    
    for(int i=0, j=1; i<to_rep.size(); i++){
      while(dsu.connected(0, j)) j++;
      cout << to_rep[i].first+1 << " " << to_rep[i].second+1 << " "
        << 1 <<" "<< j+1 << endl;
      dsu.merge(0, j);
    }
