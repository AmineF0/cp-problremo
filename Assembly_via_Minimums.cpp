...
void solve(){
    ...
    for(auto val : m){
      int cnt = 0;
      while(weird(cnt, k) < val.second) cnt++;
      // cout << k << " " << val.first << " " << val.second << " " << cnt << endl;
      for(int i=0; i<cnt; i++) ans.push_back(val.first);
      tot+=cnt;
      k--;
    }
    // print(v);
        for(int i=0; i<n-tot; i++) cout << ans[ans.size()-1] << " ";
